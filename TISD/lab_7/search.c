...
int if_transfer_exists(int **matrix1, int **matrix2, int n, int point)
{
    int count = 0; // expected non zero means
    for (int j = 0; j < n; j++)
    {
        if (matrix1[point][j] != 0 && matrix2[point][j] != 0)
        {
            count++;
        }
    }
...
void search_Dijkstra(int **VES, int n, int start, int end, int **matrix2)
{
    ...
    for (int prosto = 0; prosto < VERTEX; prosto++)
    {
        ...
        else
        {
            ...
            while(1)
            {
                ...
                if (v == -1)
                {
                    //printf("Такой путь не найден!\n");
                    break;
                }

                if (v == end && end + 1 == end_copy)                            // Найден кратчайший путь,
                {                                 // выводим его
                    printf("Кратчайший путь из вершины %d в вершину %d :\n", start + 1, end + 1);
                    u = end;
                    while(u != start)
                    {
                        true_or_false = if_transfer_exists(VES, matrix2, n, u);
                        if (true_or_false == TRUE)
                        {
                            printf(" %d (можно пересесть)\n", u + 1);
                        }
                        else
                        {
                            printf(" %d\n", u + 1);
